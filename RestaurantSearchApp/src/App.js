...
const App = () => {
    ...
    useEffect(() => {
        if (bounds.sw && bounds.ne) {
            ...
            getPlacesData(type, bounds.sw, bounds.ne).then((data) => { console.log("data",data); setPlaces(data?.filter((place) => place.name && place.num_reviews > 0)); setFilteredPlaces([]); setIsLoading(false) });
        }
    }, [type, bounds]);
    console.log(places);
